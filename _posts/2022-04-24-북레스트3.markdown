...
<img style="margin-left: 0; width: 90%;" src="https://github.com/gitul0515/gitul0515.github.io/blob/main/_posts/image/bookrest/3_1.png?raw=true" alt="documentFragment">
...
여기서는 전체적인 로직을 중심으로 설명하겠다.

Controller는 하나의 객체이며, 그 안에 여러 메서드를 소유한다.  
가장 중요한 메서드는 init이다.  
init은 컨트롤러를 초기화하는 역할을 하며,  
'한 번만 선언되는 영역'과 '중복 선언되는 영역'으로 나뉜다.

#### 1) 단일 선언

한 번만 선언되는 영역에는 setupInnerPage와 addCustomEvent가 있다.  
setupInnerPage는 내부페이지를 Setup하는 역할이다.
내부페이지가 무엇이냐구?  
우선 아래 영상을 봐주기 바란다.
